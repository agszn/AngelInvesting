...
def view_profile(request):
    ...
    return render(request, 'accounts/profile.html', {'profile': profile, 'form': form,'brokers':brokers})
...
@csrf_exempt
@login_required
def CMR_Details(request):
    brokers = Broker.objects.all()
    print
    cmr_copies = CMRCopy.objects.filter(user_profile=request.user.profile)
    ...
    if request.method == 'POST':
        ...
        if cmr_id:
            cmr = get_object_or_404(CMRCopy, id=cmr_id)
            # Optional: add permission check if needed, to ensure user owns this CMR
            if cmr.user_profile != request.user.profile:
                messages.error(request, "You do not have permission to edit this CMR copy.")
                return redirect('cmr_details')
        else:
            cmr = CMRCopy()

        cmr.user_profile = request.user.profile  # assign logged-in user profile

        if not broker_id:
            messages.error(request, "Please select a broker before saving.")
            return redirect('cmr_details')

        try:
            cmr.broker = Broker.objects.get(broker_id=broker_id)
        except Broker.DoesNotExist:
            messages.error(request, "Selected broker not found.")
            return redirect('cmr_details')

        # Save raw input IDs as well
        ...
        cmr.save()
        messages.success(request, "CMR saved successfully.")
        return redirect('cmr_details')
...
def delete_cmr(request, cmr_id):
    if request.method == 'POST':
        cmr = get_object_or_404(CMRCopy, id=cmr_id)
        cmr.delete()
        messages.success(request, "CMR copy deleted successfully.")
    return redirect('cmr_details')  # Replace with your actual page name

@login_required
def download_cmr_file(request, cmr_id):
    # Get the CMR record
    try:
        cmr = CMRCopy.objects.get(id=cmr_id, client=request.user.profile)
